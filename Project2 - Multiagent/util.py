...
def nSample(distribution, values, n):
    ...
    rand.sort()
    samples = []
    samplepacman_pos, distpacman_pos, cdf = 0, 0, distribution[0]
    while samplepacman_pos < n:
        if rand[samplepacman_pos] < cdf:
            samplepacman_pos += 1
            samples.append(values[distpacman_pos])
        else:
            distpacman_pos += 1
            cdf += distribution[distpacman_pos]
    return samples
...
def nearestPoint(pacman_pos):
    """
    Finds the nearest grid point to a pacman_position (discretizes).
    """
    (current_row, current_col) = pacman_pos
