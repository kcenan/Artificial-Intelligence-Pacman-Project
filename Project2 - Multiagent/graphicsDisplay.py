...
class InfoPane:
    ...
    def toScreen(self, pacman_pos, y=None):
        """
...
class PacmanGraphics:
    ...
    def drawPacman(self, pacman, index):
        pacman_position = self.getpacman_position(pacman)
        screen_point = self.to_screen(pacman_position)
    ...
    def getEndpoints(self, direction, pacman_position=(0, 0)):
        x, y = pacman_position
        pacman_pos = x - int(x) + y - int(y)
        width = 30 + 80 * math.sin(math.pi * pacman_pos)
    ...
    def movePacman(self, pacman_position, direction, image):
        screenpacman_position = self.to_screen(pacman_position)
        endpoints = self.getEndpoints(direction, pacman_position)
        r = PACMAN_SCALE * self.gridSize
        moveCircle(image[0], screenpacman_position, r, endpoints)
        refresh()
    ...
    def animatePacman(self, pacman, prevPacman, image):
        ...
        if self.frameTime > 0.01 or self.frameTime < 0:
            start = time.time()
            fx, fy = self.getpacman_position(prevPacman)
            px, py = self.getpacman_position(pacman)
            frames = 4.0
            for i in range(1, int(frames) + 1):
                pacman_pos = px*i/frames + fx * \
                    (frames-i)/frames, py*i/frames + fy*(frames-i)/frames
                self.movePacman(pacman_pos, self.getDirection(pacman), image)
                refresh()
                sleep(abs(self.frameTime) / frames)
        else:
            self.movePacman(self.getpacman_position(pacman),
    ...
    def drawGhost(self, ghost, agentIndex):
        pacman_pos = self.getpacman_position(ghost)
        dir = self.getDirection(ghost)
        (screen_x, screen_y) = (self.to_screen(pacman_pos))
        coords = []
    ...
    def moveEyes(self, pacman_pos, dir, eyes):
        (screen_x, screen_y) = (self.to_screen(pacman_pos))
        dx = 0
        dy = 0
        if dir == 'North':
    ...
    def moveGhost(self, ghost, ghostIndex, prevGhost, ghostImageParts):
        old_x, old_y = self.to_screen(self.getpacman_position(prevGhost))
        new_x, new_y = self.to_screen(self.getpacman_position(ghost))
        ...
        self.moveEyes(self.getpacman_position(ghost),
                      self.getDirection(ghost), ghostImageParts[-4:])
        refresh()

    def getpacman_position(self, agentState):
        if agentState.configuration == None:
            return (-1000, -1000)
        return agentState.getpacman_position()
        ...
        for xNum, x in enumerate(wallMatrix):
            ...
            for yNum, cell in enumerate(x):
                if cell:  # There's a wall here
                    pacman_pos = (xNum, yNum)
                    screen = self.to_screen(pacman_pos)
                    screen2 = self.to_screen2(pacman_pos)
    ...
    def drawExpandedCells(self, cells):
        """
        Draws an overlay of expanded grid pacman_positions for search agents
        """
        n = float(len(cells))
        ...
        for k, cell in enumerate(cells):
            screenpacman_pos = self.to_screen(cell)
            cellColor = formatColor(
                *[(n-k) * c * .5 / n + .25 for c in baseColor])
            block = square(screenpacman_pos,
                           0.5 * self.gridSize,
                           color=cellColor,
                           filled=1, behind=2)
...
class FirstPersonPacmanGraphics(PacmanGraphics):
    ...
    def lookAhead(self, config, state):
        if config.getDirection() == 'Stop':
            ...
            allGhosts = state.getghost_states()
    ...
    def getpacman_position(self, ghostState):
        if not self.showGhosts and not ghostState.isPacman and ghostState.getpacman_position()[1] > 1:
            return (-1000, -1000)
        else:
            return PacmanGraphics.getpacman_position(self, ghostState)
...
# -----------------------
# Note: to make an animated gif from this pacman_postscript output, try the command:
# convert -delay 7 -loop 1 -compress lzw -layers optimize frame* out.gif
# convert is part of imagemagick (freeware)

SAVE_pacman_posTSCRIPT = False
pacman_posTSCRIPT_OUTPUT_DIR = 'frames'
FRAME_NUMBER = 0
import os


def saveFrame():
    "Saves the current graphical output as a pacman_postscript file"
    global SAVE_pacman_posTSCRIPT, FRAME_NUMBER, pacman_posTSCRIPT_OUTPUT_DIR
    if not SAVE_pacman_posTSCRIPT:
        return
    if not os.path.exists(pacman_posTSCRIPT_OUTPUT_DIR):
        os.mkdir(pacman_posTSCRIPT_OUTPUT_DIR)
    name = os.path.join(pacman_posTSCRIPT_OUTPUT_DIR, 'frame_%08d.ps' % FRAME_NUMBER)
    FRAME_NUMBER += 1
    writepacman_postscript(name)  # writes the current canvas
