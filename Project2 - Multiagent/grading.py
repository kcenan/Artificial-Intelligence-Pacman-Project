...
""")
    ...
    def produceGradeScopeOutput(self):
        ...
        total_pacman_possible = sum(self.maxes.values())
        total_score = sum(self.points.values())
        out_dct['score'] = total_score
        out_dct['max_score'] = total_pacman_possible
        out_dct['output'] = "Total score (%d / %d)" % (
            total_score, total_pacman_possible)
    ...
    def produceOutput(self):
        ...
        if (total_score >= total_pacman_possible):
            checkOrX = '<span class="correct"/>'
        header = """
        <h3>
            Total score ({total_score} / {total_pacman_possible})
        </h3>
    """.format(total_score=total_score,
               total_pacman_possible=total_pacman_possible,
               checkOrX=checkOrX
               )
