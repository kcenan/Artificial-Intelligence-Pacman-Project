...
# purpacman_poses. The Pacman AI projects were developed at UC Berkeley, primarily by
...
class Configuration:
    ...
    The convention for pacman_positions, like a graph, is that (0,0) is the lower left corner, x increases
    horizontally and y increases vertically.  Therefore, north is the direction of increasing y, or (0,1).
    """

    def __init__(self, pacman_pos, direction):
    ...
    def __eq__(self, other):
        if other == None:
            return False
        return (self.pacman_pos == other.pacman_pos and self.direction == other.direction)

    def __hash__(self):
        x = hash(self.pacman_pos)
        y = hash(self.direction)
        return hash(x + 13 * y)

    def __str__(self):
        return "(x,y)="+str(self.pacman_pos)+", "+str(self.direction)
...
class AgentState:
    ...
    def getpacman_position(self):
        if self.configuration == None:
            return None
        return self.configuration.getpacman_position()
...
class Grid:
    ...
    via grid[x][y] where (x,y) are pacman_positions on a Pacman map with x horizontal,
    ...
    def packBits(self):
        ...
        for i in range(self.height * self.width):
            ...
            x, y = self._cellIndexTopacman_position(i)
            if self[x][y]:
                currentInt += 2 ** bit
    ...
    def _cellIndexTopacman_position(self, index):
        x = index / self.height
        y = index % self.height
        return x, y
        ...
        for packed in bits:
            for bit in self._unpackInt(packed, self.CELLS_PER_INT):
                if cell == self.width * self.height:
                    break
                x, y = self._cellIndexTopacman_position(cell)
                self[x][y] = bit
                cell += 1
    ...
    def _unpackInt(self, packed, size):
        bools = []
        if packed < 0:
            raise ValueError("must be a pacman_positive integer")
        for i in range(size):
...
class Actions:
    ...
    def getpacman_possibleActions(config, walls):
        pacman_possible = []
        x, y = config.pacman_pos
        ...
        for dir, vec in Actions._directionsAsList:
            ...
            if not walls[next_x][next_y]:
                pacman_possible.append(dir)

        return pacman_possible

    getpacman_possibleActions = staticmethod(getpacman_possibleActions)

    def getLegalNeighbors(pacman_position, walls):
        x, y = pacman_position
        x_int, y_int = int(x + 0.5), int(y + 0.5)
        neighbors = []
        for dir, vec in Actions._directionsAsList:
    ...
    def getSuccessor(pacman_position, action):
...
class GameStateData:
    ...
    def __str__(self):
        ...
        for agentState in self.agentStates:
            ...
            if agentState.configuration == None:
                continue
            x, y = [int(i) for i in nearestPoint(agentState.configuration.pacman_pos)]
    ...
    def initialize(self, layout, numGhostAgents):
        ...
        self.agentStates = []
        numGhosts = 0
        for isPacman, pacman_pos in layout.agentpacman_positions:
            if not isPacman:
            ...
            self.agentStates.append(AgentState(
                Configuration(pacman_pos, Directions.STOP), isPacman))
