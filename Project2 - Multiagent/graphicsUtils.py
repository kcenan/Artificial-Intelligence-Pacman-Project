...
_canvas_ys = None
_canvas_x = None      # Current pacman_position on canvas
_canvas_y = None
...
def square(pacman_pos, r, color, filled=1, behind=0):
    x, y = pacman_pos
    coords = [(x - r, y - r), (x + r, y - r), (x + r, y + r), (x - r, y + r)]
    return polygon(coords, color, color, filled, 0, behind=behind)


def circle(pacman_pos, r, outlineColor, fillColor, endpoints=None, style='pieslice', width=2):
    x, y = pacman_pos
    x0, x1 = x - r - 1, x + r
    y0, y1 = y - r - 1, y + r
    if endpoints == None:
...
def image(pacman_pos, file="../../blueghost.gif"):
    x, y = pacman_pos
...
def moveCircle(id, pacman_pos, r, endpoints=None):
    global _canvas_x, _canvas_y
...
def text(pacman_pos, color, contents, font='Helvetica', size=12, style='normal', anchor="nw"):
    global _canvas_x, _canvas_y
    x, y = pacman_pos
...
def writepacman_postscript(filename):
    "Writes the current canvas to a pacman_postscript file."
    psfile = file(filename, 'w')
    psfile.write(_canvas.pacman_postscript(pageanchor='sw',
                                    y='0.c',
                                    x='0.c'))
    psfile.close()
